...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    ...
    try {
      ...
      toastSuccess('Login successful!', { id: toastId });
    } catch (error) {
      // [THE FIX] Check for the specific network error code
      if (error.code === 'auth/network-request-failed') {
        toastError('Could not connect. Please check your internet connection.', { id: toastId });
      } else if (error.code === 'auth/invalid-credential') {
        toastError('Invalid email or password. Please try again.', { id: toastId });
      } else {
        toastError(error.message, { id: toastId });
      }
      throw error;
    }
  };
  ...
  const register = async (email, password, fullName, country_code, phone_number) => {
    ...
    try {
      ...
      toastSuccess('Registration successful! Welcome.', { id: toastId });
        ...
        if (error.code === 'auth/network-request-failed') {
            toastError('Could not connect. Please check your internet connection.', { id: toastId });
             ...
             toastError('This email address is already registered.', { id: toastId });
        } else {
            toastError(error.response?.data?.detail || error.message, { id: toastId });
        }
      throw error;
    }
  };
  ...
  const resetPassword = async (email) => {
   ...
   const switchToBusiness = () => {
        if (dbUser?.business_profile) {
            ...
            toastSuccess(`Switched to ${dbUser.business_profile.business_name}`);
        }
    };
