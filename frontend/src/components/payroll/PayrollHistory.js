...
const PayrollHistory = ({ payrollRuns, isLoading, error, onExecute }) => {
    ...
    if (error) return <p className="text-center text-red-400">Could not load payroll history.</p>;
                    ...
                    {payrollRuns?.length > 0 ? payrollRuns.map(run => (
                        <tr key={run.id}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-white">
                                {new Intl.NumberFormat('en-US').format(run.total_source_cost)} {run.source_currency}
                            </td>
                        ...
                        <tr>
                            <td colSpan="4" className="text-center py-12 text-neutral-500">No payroll runs have been created yet.</td>
                        </tr>
                    )}
                </tbody>
            </table>
