...
const FormInput = ({ label, type = 'text', ...props }) => {
  ...
  return (
    <div className="w-full">
      {label && (
        <label htmlFor={props.id || props.name} className="block text-sm font-medium text-neutral-300 mb-1">
          {label}
        </label>
      )}
      <div className="relative">
        <input
          ...
          className={`
            ...
            ${isError ? 'border-red-500 focus:ring-red-500' : 'border-neutral-700 focus:ring-primary'}
          `}
        />
        {type === 'password' && (
          <button
