...
const MomoDeposit = ({ onDepositInitiated }) => {
    ...
    const handleSubmit = async (values) => {
        ...
        if (result.success) {
            toast.success(result.data.message || "Request sent to your phone!");
            setIsSubmitted(true);
    ...
    if (isSubmitted) {
        return (
            <div className="p-8 text-center space-y-4">
                ...
                <p className="text-neutral-300">Please approve the payment request sent to your mobile money phone to complete the deposit.</p>
            </div>
        )
    }
                ...
                {({ errors, touched }) => (
                    <Form className="space-y-4">
                        ...
                        <div>
                            ...
                            <Field as="select" name="network" className="block w-full px-3 py-2 bg-neutral-800 border border-neutral-700 rounded-md text-white">
