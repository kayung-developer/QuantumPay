...
import MomoDeposit from './MomoDeposit';
import EftDeposit from './EftDeposit';
import PlaidLink from './PlaidLink'; // <-- IMPORT THE NEW PLAID COMPONENT

// Simple SVG for M-Pesa logo
const SvgMpesa = () => (
...
const DepositModal = ({ isOpen, onClose, wallet, onDepositSuccess }) => {
    ...
    const availableMethods = useMemo(() => {
        ...
        // [THE IMPLEMENTATION] The complete, Pan-African & Global list of payment methods
        const allMethods = [
    ...
    useEffect(() => {
        // This robust effect ensures a default is set when the modal opens or the available methods change.
        if (isOpen && availableMethods.length > 0) {
            // Check if the currently selected method is still valid for the new wallet
            ...
            setSelectedMethod(''); // Reset when modal closes
        }
    ...
    const renderMethodContent = () => {
        switch (selectedMethod) {
            ...
            case 'open_banking': // [THE IMPLEMENTATION]
                return <PlaidLink onLinkSuccess={onDepositSuccess} />;
            default:
                return (
                    <div className="p-8 text-center text-neutral-400 h-full flex flex-col justify-center">
            ...
            {wallet ? (
                 <div className="flex flex-col md:flex-row gap-6 items-start">
                             ...
                             {availableMethods.map(method => (
                                <button
                                    ...
                                    className={`w-full flex items-center p-3 rounded-lg text-left transition-colors ${
                                        selectedMethod === method.id ? 'bg-primary/20' : 'hover:bg-neutral-800'
                                    }`}
                                >
                                    <method.icon className={`h-6 w-6 mr-3 ${selectedMethod === method.id ? 'text-primary' : 'text-neutral-400'}`} />
                    ...
                    <div className="w-full md:flex-1">
                        <div className="border border-neutral-800 rounded-lg min-h-[350px] flex flex-col">
                            {renderMethodContent()}
                        </div>
                    </div>
                 </div>
