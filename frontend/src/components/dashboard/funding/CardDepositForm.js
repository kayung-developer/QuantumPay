...
const CardDepositForm = ({ onDepositSuccess }) => {
    ...
    const handleCharge = async (values, { setFieldError }) => {
        ...
        if (result.success) {
            ...
            // The useApiPost hook shows a generic toast, but we can set a specific field error.
            setFieldError('number', result.error || 'This card was declined by the processor.');
        }
    };
    ...
    const handleVerification = async (reference) => {
        ...
        if (result.success) {
            ...
            toast.error(result.error || "Verification failed. If funds were debited, please contact support.");
            setChargeState({ status: 'form', message: '', reference: '' }); // Reset form on failure
        }
    };
