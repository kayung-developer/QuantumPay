...
const CreateInvoiceModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  return (
    ...
    <div>
        <label htmlFor="currency" className="block text-sm font-medium text-neutral-300 mb-1">
            Currency
        </label>
        <Field
            as="select"
            name="currency"
            id="currency"
            className="block w-full px-3 py-2 bg-neutral-800 border border-neutral-700 rounded-md text-white placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-primary"
        >
...
/>
            ...
            <div className="border-t border-neutral-800 pt-4">
              <h3 className="text-lg font-medium text-white mb-2">Invoice Items</h3>
              <FieldArray name="items">
                {({ push, remove }) => (
                  <div className="space-y-4">
                    {values.items.map((item, index) => (
                      <div key={index} className="flex items-start gap-4 p-4 bg-neutral-800/50 rounded-md">
                        <div className="grid grid-cols-12 gap-4 flex-grow">
                          <div className="col-span-6">
                             <FormInput name={`items.${index}.description`} placeholder="Item description"/>
                          </div>
                           <div className="col-span-2">
                             <FormInput name={`items.${index}.quantity`} type="number" placeholder="Qty"/>
                          </div>
                           <div className="col-span-4">
                             <FormInput name={`items.${index}.unit_price`} type="number" placeholder="Unit Price"/>
                          </div>
                        </div>
                        <Button type="button" variant="danger" size="sm" onClick={() => remove(index)} className="mt-1 p-2">
