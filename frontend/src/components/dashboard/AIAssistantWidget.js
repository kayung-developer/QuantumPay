            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: 50 }}
                        className="fixed bottom-24 right-6 z-50 w-full max-w-sm h-[60vh] bg-neutral-900/80 backdrop-blur-xl border border-neutral-700 rounded-2xl shadow-2xl flex flex-col"
                    >
                        {/* Header */}
                        <div className="flex items-center justify-between p-4 border-b border-neutral-700">
                            <h3 className="font-semibold text-white flex items-center"><SparklesIcon className="h-5 w-5 mr-2 text-primary"/> QuantumAI Assistant</h3>
                            <button onClick={() => setIsOpen(false)} className="text-neutral-400 hover:text-white"><XMarkIcon className="h-5 w-5"/></button>
                        </div>
                        ...
                        <div className="flex-grow p-4 overflow-y-auto space-y-4">
                            {messages.map((msg, index) => (
                                <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-xs px-4 py-2 rounded-2xl ${msg.role === 'user' ? 'bg-primary text-white' : 'bg-neutral-800 text-neutral-200'}`}>
                        ...
                        <div className="p-4 border-t border-neutral-700">
                            <form onSubmit={handleSend} className="flex items-center space-x-2">
                                <input
                                    ...
                                    className="flex-grow bg-neutral-800 border-neutral-700 rounded-full px-4 py-2 focus:ring-primary focus:border-primary text-sm"
                                    disabled={loading}
                                />
