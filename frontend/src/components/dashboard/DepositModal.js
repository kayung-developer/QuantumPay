...
const DepositModal = ({ isOpen, onClose, wallet, onSuccess }) => {
  const { post: initializeDeposit, loading, data } = useApiPost('/wallets/deposit/initialize');
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    if (result.success && result.data?.data?.authorization_url) {
      ...
      toast.success('Deposit initialized! Please complete the payment.');
    }
    setSubmitting(false);
  };
    ...
    if (paymentUrl) {
      return (
        <div className="text-center">
            ...
            <div className="mt-4 p-3 bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-md text-primary break-all">
