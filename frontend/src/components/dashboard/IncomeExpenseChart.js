...
const IncomeExpenseChart = ({ data: chartData, isLoading, error }) => {
    ...
    const renderContent = () => {
        ...
        if (error) return <p className="text-center text-red-500">{t('chart_load_error') || 'Could not load chart data.'}</p>;
        if (!chartData || !chartData.labels || chartData.labels.length === 0) return <p className="text-center text-sm text-neutral-500 dark:text-neutral-400">{t('chart_empty_message')}</p>;
        return <Line ref={chartRef} options={chartOptions} data={data} />;
    };

    return (
        <div className="bg-white dark:bg-neutral-900 p-6 rounded-lg shadow border border-neutral-200 dark:border-neutral-800 h-full flex flex-col">
            <h3 className="text-lg font-semibold text-neutral-900 dark:text-white font-display flex-shrink-0">{t('chart_title')}</h3>
            <div className="mt-4 flex-grow h-80">{renderContent()}</div>
        </div>
    );
};
