 ...
 * It is "auth-aware" and will wait for an auth token before firing.
 */
export const useApi = (url, options = {}, manual = false) => {
    ...
    const request = useCallback(async (requestOptions) => {
        if (!authToken) {
            return { success: false, error: { message: "Auth token not available." } };
        }
        setLoading(true);
        setError(null);
        try {
 ...
 * It is now "auth-aware" and will fail gracefully if no token is present.
 */
export const useApiPost = (url, config = {}) => {
    ...
    const post = useCallback(async (postData, requestConfig = {}) => {
        ...
        // This guard clause is the most critical fix. It prevents any API call
        // from being made if the authentication token hasn't loaded yet.
        if (!authToken) {
