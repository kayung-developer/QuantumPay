...
export const useApiPost = (url, config = {}) => {
    ...
    const post = useCallback(async (postData, requestConfig = {}) => {
      ...
      try {
        ...
        // Determine the HTTP method from the config, defaulting to 'POST'
        const method = finalConfig.method ? finalConfig.method.toLowerCase() : 'post';

        let response;
        // Use the appropriate axios method
        if (method === 'put') {
            response = await apiClient.put(url, postData, finalConfig);
        } else if (method === 'delete') {
            // For DELETE, data is often not in the body but config can be passed
            response = await apiClient.delete(finalConfig.url || url, finalConfig);
        } else {
            response = await apiClient.post(url, postData, finalConfig);
        }

        setData(response.data);

        // For DELETE requests, we often get a 204 No Content, so we don't assume a message.
