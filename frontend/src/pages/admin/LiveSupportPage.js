...
// --- [THEME-AWARE] A sub-component for a cleaner, more readable layout ---
const ChatMessage = ({ msg, currentAdminId }) => {
    ...
    return (
        <div className={`flex items-end gap-2 ${isSentByAdmin ? 'justify-end' : 'justify-start'}`}>
            <div className={`flex flex-col space-y-2 text-sm max-w-xs mx-2 ${isSentByAdmin ? 'order-2 items-end' : 'order-1 items-start'}`}>
                <div className={`px-4 py-2 rounded-2xl ${isSentByAdmin ? 'rounded-br-none bg-primary text-white' : 'rounded-bl-none bg-neutral-200 dark:bg-neutral-800 text-neutral-900 dark:text-white'}`}>
                    <p>{msg.content}</p>
                </div>
            </div>
        </div>
...
const LiveSupportPage = () => {
    ...
    useEffect(() => {
        if (selectedConvo) {
            ...
            // When no conversation is selected, clear the messages
            setMessages([]);
        }
    }, [selectedConvo]);
    ...
    const handleSend = (e) => {
        ...
        if (!input.value.trim() || !selectedConvo || !ws.current || ws.current.readyState !== WebSocket.OPEN) return;
    ...
    const renderConversationList = () => {
        ...
        return conversations.map((convo) => (
            <button
                ...
                className={`w-full text-left p-3 rounded-md transition-colors ${selectedConvoId === convo.id ? 'bg-primary/10' : 'hover:bg-neutral-100 dark:hover:bg-neutral-800'}`}
            >
                <div className="flex items-center justify-between">
                    ...
                    {/* --- [THE FEATURE IMPLEMENTATION] Conditionally render the Priority badge --- */}
                    {convo.is_priority && (
                    ...
                    {selectedConvo ? (
                        <>
                            ...
                            <div className="flex-grow p-4 overflow-y-auto space-y-4">
                                {messages.map((msg) => (
                                    <ChatMessage key={msg.id} msg={msg} currentAdminId={dbUser?.id} />
                                ))}
                                <div ref={messagesEndRef} />
                            </div>
                            <div className="p-4 border-t border-neutral-200 dark:border-neutral-700 flex-shrink-0">
                                <form onSubmit={handleSend} className="flex items-center space-x-3">
                                    <input name="message" placeholder="Type your message..." autoComplete="off" className="flex-grow bg-neutral-100 dark:bg-neutral-800 border-transparent focus:ring-2 focus:ring-primary rounded-full px-4 py-2 text-neutral-900 dark:text-white" />
                                    <button type="submit" className="p-3 bg-primary rounded-full text-white hover:bg-primary-dark transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary dark:focus:ring-offset-neutral-900">
                         ...
                         <div className="flex flex-col justify-center items-center h-full text-neutral-500 dark:text-neutral-400 p-8 text-center">
                            <ChatBubbleLeftRightIcon className="h-16 w-16 text-neutral-400" />
