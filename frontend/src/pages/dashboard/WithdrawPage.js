// FILE: src/pages/dashboard/WithdrawPage.js
...
const WithdrawPage = () => {
    ...
    const handlePayoutSuccess = () => {
        ...
        refetch(); // Refetch wallets/balances might be needed here too
    };
    ...
    const renderContent = () => {
        ...
        if (error) {
            return <div className="text-center py-16 text-red-400"><p>Could not load your saved bank accounts.</p></div>;
        }
    ...
    return (
        <DashboardLayout pageTitleKey="withdraw_funds_title">
        ...
        >
            ...
            <Menu.Items className="absolute right-0 mt-2 w-64 origin-top-right rounded-md bg-white dark:bg-neutral-800 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none border border-neutral-200 dark:border-neutral-700">
                <div className="py-1">
                    {/* --- Pan-African Payouts --- */}
                    <div className="px-4 py-2 text-xs font-semibold text-neutral-500">Pan-African Payouts</div>
                    <Menu.Item>
                        {({ active }) => (
                            <button
                                onClick={() => { setCurrencyToAdd('NGN'); setIsAddModalOpen(true); }}
                                className={`${active ? 'bg-neutral-100 dark:bg-neutral-700' : ''} group flex w-full items-center px-4 py-2 text-sm text-neutral-700 dark:text-neutral-200`}
                            >
                                <span className="fi fi-ng mr-3"></span>
                                Nigerian Bank (NGN)
                            </button>
                        )}
                    </Menu.Item>
                    <Menu.Item>
                        {({ active }) => (
                            <button
                                ...
                                className={`${active ? 'bg-neutral-100 dark:bg-neutral-700' : ''} group flex w-full items-center px-4 py-2 text-sm text-neutral-700 dark:text-neutral-200`}
                            >
                                <span className="fi fi-gh mr-3"></span>
                                Ghanaian Bank (GHS)
                            </button>
                        )}
                    </Menu.Item>
                    <Menu.Item>
                        {({ active }) => (
                            <button
                                ...
                                className={`${active ? 'bg-neutral-100 dark:bg-neutral-700' : ''} group flex w-full items-center px-4 py-2 text-sm text-neutral-700 dark:text-neutral-200`}
                            >
                                <span className="fi fi-ke mr-3"></span>
                                Kenyan Bank (KES)
                            </button>
                        )}
                    </Menu.Item>
                    <Menu.Item>
                        {({ active }) => (
                            <button
                                ...
                                className={`${active ? 'bg-neutral-100 dark:bg-neutral-700' : ''} group flex w-full items-center px-4 py-2 text-sm text-neutral-700 dark:text-neutral-200`}
                            >
                                 <span className="fi fi-za mr-3"></span>
                    ...
                    {/* --- Global Payouts --- */}
                    <div className="px-4 py-2 text-xs font-semibold text-neutral-500">Global Payouts</div>
                    <Menu.Item>
                        {({ active }) => (
                            <button
                    ...
                    <Menu.Item>
                        {({ active }) => (
                            <button
                                ...
                                className={`${active ? 'bg-neutral-100 dark:bg-neutral-700' : ''} group flex w-full items-center px-4 py-2 text-sm text-neutral-700 dark:text-neutral-200`}
                            >
                                 <span className="fi fi-gb mr-3"></span>
                                British Bank (GBP)
                            </button>
                        )}
                    </Menu.Item>
                    <Menu.Item>
                        {({ active }) => (
                            <button
                                ...
                                className={`${active ? 'bg-neutral-100 dark:bg-neutral-700' : ''} group flex w-full items-center px-4 py-2 text-sm text-neutral-700 dark:text-neutral-200`}
                            >
                                 <span className="fi fi-eu mr-3"></span>
            ...
            <AddBankAccountModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onSuccess={handleAddSuccess} currency={currencyToAdd} />
            {selectedAccount && <PayoutConfirmationModal isOpen={!!selectedAccount} onClose={() => setSelectedAccount(null)} account={selectedAccount} onSuccess={handlePayoutSuccess} />}
        </DashboardLayout>
    );
};
