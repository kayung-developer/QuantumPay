// FILE: src/pages/dashboard/KYCCenterPage.js
        ...
        >
            {({ errors, touched }) => (
                 <Form className="space-y-6">
                    <div>
                        <label htmlFor="document_type" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Document Type</label>
...
const KYCCenterPage = () => {
    ...
    // Conditionally fetch rules only if the user needs to submit documents.
    ...
    const { data: kycRules, loading: rulesLoading, error: rulesError } = useApi(
        ...
        {},
        !shouldFetchRules // `manual` is true if we should NOT fetch
    );
