...
// --- [THE UPGRADE] Issue Card Modal with Tier Selection ---
const IssueCardModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    const handleSubmit = async (values) => {
        const payload = { ...values, card_type: 'VIRTUAL' }; // Assuming all are virtual for now
                        ...
                        <div>
                                    ...
                                    {cardTiers.map((card) => (
                                        <RadioGroup.Option
                                            ...
                                            className={({ active, checked }) =>
                                                ...
                                                ${!card.available ? 'bg-neutral-100 dark:bg-neutral-800 opacity-50' : 'bg-white dark:bg-neutral-900'}
                                                ${checked ? 'border-transparent ring-2 ring-primary' : 'border-neutral-300 dark:border-neutral-700'}
                                                ...
                                                <>
                                                    <div className="flex flex-1">
                                                        <div className="flex flex-col">
                                                            <RadioGroup.Label as="span" className="block text-sm font-medium text-neutral-900 dark:text-white">{card.name}</RadioGroup.Label>
                                                            <RadioGroup.Description as="span" className="mt-1 flex items-center text-sm text-neutral-500 dark:text-neutral-400">{card.description}</RadioGroup.Description>
                                                        </div>
                                                    </div>
                            ...
                            {!isUltimateSubscriber && (
                                <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400">
                                    <Link to="/pricing" className="text-primary hover:underline">{t('upgrade_for_premium')}</Link>
                                </p>
                            )}
                        </div>
    ...
    return (
        <DashboardLayout pageTitleKey="corporate_cards_title">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {cards?.map(card => (
                    <div key={card.id} className={`p-6 rounded-xl shadow-md ${card.card_tier === 'premium' ? 'bg-gradient-to-br from-neutral-800 to-black dark:from-neutral-900 dark:to-black text-white' : 'bg-white dark:bg-neutral-900'}`}>
