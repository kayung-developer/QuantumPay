...
const PayrollPage = () => {
    ...
    const handleEmployeeSave = (isEditing) => {
        ...
        refetchEmployees();
        toastSuccess(`Team member ${isEditing ? 'updated' : 'added'} successfully.`);
    };
    ...
    const handleDeleteEmployee = (employee) => {
        if (window.confirm(`Are you sure you want to remove ${employee.user.full_name} from your team? This will make them inactive.`)) {
            deleteEmployee({}, { url: `/business/employees/${employee.id}` }).then(result => {
                if (result.success) {
                    toastSuccess(result.data.message || "Team member updated.");
    ...
    const handleRunCreated = () => {
        ...
        refetchRuns();
        toastSuccess("Payroll run calculated and saved as pending.");
    };
    ...
    const handleRunExecuted = () => {
        ...
        toastSuccess("Payroll has been executed successfully!");
    };
