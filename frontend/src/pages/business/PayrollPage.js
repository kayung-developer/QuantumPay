...
const PayrollPage = () => {
    ...
    const { dbUser } = useAuth(); // Get the authenticated user with subscription info
    ...
    const planLimits = useMemo(() => {
        ...
        if (planId === 'ultimate') return { max_team_members: 1000 }; // Effectively unlimited
        if (planId === 'premium') return { max_team_members: 10 };
            ...
            <div className="space-y-8">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between">
                    ...
                    <div className="mt-4 md:mt-0 flex flex-col md:flex-row items-end md:items-center space-y-2 md:space-y-0 md:space-x-3">
                        {/* [THE UPGRADE] Conditionally render the 'Invite Member' button or an 'Upgrade' prompt */}
                        {hasReachedTeamLimit ? (
                             <div className="text-right">
                                ...
                                <Link to="/pricing" className="text-sm text-primary hover:underline">
                                    Upgrade to add more members &rarr;
                                </Link>
                            </div>
                        ) : (
