...
from sqlalchemy import (
    ...
    Boolean, Enum as SQLAlchemyEnum, Text, Integer, event, Table, true, func, case, extract
)
...
# V4.4 - Association Table for Shared Vaults and Users (Many-to-Many)
vault_members_table = Table(
    "vault_members",
    Base.metadata,
    Column("vault_id", String, ForeignKey("shared_vaults.id"), primary_key=True),
    Column("user_id", String, ForeignKey("users.id"), primary_key=True),
    Column("role", String, default="member") # e.g., 'member', 'admin'
)
...
class TreasuryRuleAction(str, Enum):
    SWEEP = "sweep"
    CONVERT = "convert"
...
class TransactionType(str, Enum):
    ...
    PAYMENT = "payment"  # For merchant sales
...
class DisputeStatus(str, Enum):
    ...
    RESOLVED_FAVOR_USER = "resolved_favor_user"
...
class User(Base):
    ...
    credit_score = Column(Integer, default=300)  # Scores from 300 to 850
    preferred_display_currency = Column(String(3), default="USD")
    is_2fa_enabled = Column(Boolean, default=False)
    totp_secret = Column(String, nullable=True)
    notification_preferences = Column(Text, default='{"email_transactions": true, "email_security": true}')  # JSON blob
    ...
    reviewed_kyc_records = relationship(
        "KYCRecord",
        foreign_keys="[KYCRecord.reviewed_by]",
...
class Wallet(Base):
    ...
    sub_accounts = relationship("SubAccount", back_populates="wallet", cascade="all, delete-orphan")
...
class Expense(Base):
    ...
    employee_user = relationship("User", back_populates="expenses", foreign_keys=[employee_id])
    reviewer_user = relationship("User", back_populates="reviewed_expenses", foreign_keys=[reviewed_by])
...
class SubAccount(Base):
    ...
    wallet = relationship("Wallet", back_populates="sub_accounts", cascade="all, delete-orphan", single_parent=True)
...
class UserLinkedBankAccount(Base):
    ...
    provider_recipient_code = Column(String, unique=True, nullable=False, index=True)
    bank_name = Column(String)
    account_name = Column(String)
...
class InvoiceCreate(BaseModel):
    ...
    tax_rate: Optional[float] = Field(None, ge=0, le=100)  # Tax rate in percentage
...
class BlogPost(BaseModel):
    ...
    image_url: str
    summary: str
    content: str # In a real system, this could be Markdown or HTML
...
class AbstractPaymentProvider(ABC):
    ...
    async def check_health(self):
        """
        Performs a real-time health check of the provider's API.
        This could involve pinging a dedicated status endpoint.
        """
        # For this implementation, we'll assume the health is good unless a call fails.
        # A real implementation would have a dedicated health check endpoint.
        if time.time() - self.last_checked > 60:  # Check health every 60 seconds max
             logger.info(f"Performing health check for {self.name}...")
             PING_ENDPOINT = "..."
             try:
               async with httpx.AsyncClient(timeout=5) as client:
                   await client.get(PING_ENDPOINT)
               self.status = "operational"
             except:
    ...
    async def handle_flutterwave_webhook(self, db: Session, event_data: dict):
        ...
        if event_type == "charge.completed":
            ...
            if data.get("status") == "successful":
                ...
                asyncio.create_task(
                    push_notification_service.send_push_notification(db, user.id, "Wallet Funded!",
                                                                     f"Your deposit of {amount} {currency} was successful.")
                )
...
class InterswitchProvider(AbstractPaymentProvider):
    ...
    def __init__(self):
        ...
        self.base_url = "https://sandbox.interswitchng.com"  # Use production URL when live
        self._token_cache = TTLCache(maxsize=1, ttl=3500)

    @cached(TTLCache(maxsize=1, ttl=3500))
    async def _get_token(self) -> str:
        """Fetches and caches the Interswitch OAuth2 access token."""
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                logger.info("Successfully fetched new Interswitch access token.")
                self.status = "operational"
                return access_token
        except (httpx.HTTPStatusError, ValueError) as e:
            logger.error(f"Interswitch auth failed: {e}")
...
class PaystackProvider(AbstractPaymentProvider):
    ...
    async def get_billers_by_category(self, category_id: str) -> List[dict]:
        ...
        """
        try:
            # Paystack's API uses 'service_type' which is equivalent to our category_id (slug)
            raw_data = await self._api_get_request(f"billers?service_type={category_id}&country=NG")
            # Standardized format: {"id": "...", "name": "...", "fee": 0.0, "requires_validation": True/False}
            return [
                {
                    "id": item["biller_code"],
                    "name": item["name"],
                    "fee": 0.0,  # Paystack fees are typically not per-item but merchant-side
                    "requires_validation": "DSTV" in item["name"].upper()  # Example logic
                }
...
class UniversalBillerService:
    ...
    def __init__(self):
        # Register all our available, implemented provider adapters.
        # This modular design means adding a new provider (e.g., eTranzact)
        # is as simple as creating its adapter and adding it to this dictionary.
        ...
        # The routing table defines the preferred order of providers for a GENERIC biller type.
        # In a production system, this would be stored in a database or a configuration
        # management system (like Consul or Spring Cloud Config) to allow for dynamic
        # updates without redeploying the application code.
        self.routing_table = {
            # Biller Category : [Primary Provider, Secondary Provider, ...]
            "tv": ["interswitch", "paystack"],
            "airtime": ["paystack", "interswitch"],
            "government": ["remita"],
            "electricity": ["interswitch"],  # Can add more backups
        }

    @cached(TTLCache(maxsize=1, ttl=1800))
    async def get_all_billers(self) -> List[dict]:
        """
        [DEFINITIVE FIX]
        Fetches billers from all available providers, gracefully handling individual
        provider failures without crashing the entire process.
        """
        all_billers = []
        unique_biller_ids = set()

        provider_category_map = {
            "interswitch": [("4", "tv"), ("6", "electricity")],
            "paystack": [("airtime", "airtime")],
            "remita": [("government_services", "government")],
        }

        async def fetch_from_provider(p_name, cat_id, cat_name):
            provider = self.providers.get(p_name)
            if not provider: return  # Skip if provider isn't registered

            # [THE FIX] Wrap the entire process for a single provider in a try/except block.
            try:
                await provider.check_health()
                if provider.status == "operational":
                    provider_billers = await provider.get_billers_by_category(cat_id)
                    for biller in provider_billers:
                        unique_key = f"{cat_name}_{biller['name'].replace(' ', '').lower()}"
                        if unique_key not in unique_biller_ids:
                            biller.update({"provider": p_name, "category": cat_name})
                            all_billers.append(biller)
                            unique_biller_ids.add(unique_key)
            except Exception as e:
                # If one provider fails (e.g., Interswitch is down), log the error but DON'T crash.
                # This allows billers from Paystack and Remita to still be returned.
                logger.error(f"Failed to fetch billers from provider '{p_name}' for category '{cat_name}'. Reason: {e}")

        tasks = []
        for provider_name, categories in provider_category_map.items():
            for cat_id, cat_name in categories:
                tasks.append(fetch_from_provider(provider_name, cat_id, cat_name))

        await asyncio.gather(*tasks)

        if not all_billers:
            logger.warning("No billers could be fetched from any active provider.")

        return all_billers

    async def validate_customer(self, biller_id: str, customer_id: str, provider_name: str) -> dict:
        """
        Validates a customer using a SPECIFIC provider, as determined by the frontend.
        The frontend knows the provider from the `get_all_billers` call.
        """
        provider = self.providers.get(provider_name)
        if not provider:
            raise HTTPException(status_code=400, detail=f"Provider '{provider_name}' is not configured.")

        return await provider.validate_customer(biller_id, customer_id)
    ...
    async def pay_bill(self, db: Session, user: User, biller_id: str, amount: float, customer_id: str,
        ...
        """
        [REAL-SYSTEM IMPLEMENTATION]
        Intelligently processes a bill payment. It determines the optimal provider routing,
        attempts the transaction, and automatically fails over to a secondary provider if needed.
        """
        # 1. Determine the routing order
        preferred_providers = self.routing_table.get(biller_category, [provider_name])

        # 2. Pre-flight checks (Wallet Balance)
        # Fetch the fee from the specific biller object from a cached list or DB
        # For simplicity, we'll use a default fee here.
        fee = 100.0 if biller_category in ["tv", "electricity"] else 157.50 if biller_category == "government" else 0.0
        total_amount = amount + fee

        wallet = WalletService.get_user_wallet(db, user.id, "NGN")
        if not wallet or wallet.balance < total_amount:
            raise HTTPException(status_code=402, detail="Insufficient NGN balance to cover the bill and service fee.")
        ...
        for provider_name_attempt in preferred_providers:
            provider = self.providers[provider_name_attempt]
            await provider.check_health()
            if provider.status == "operational":
                try:
                    request_id = f"QPAY_{uuid.uuid4().hex[:12]}"
                    logger.info(f"Attempting payment for biller {biller_id} via primary provider: {provider.name}")
                    payment_result = await provider.make_payment(biller_id, amount, customer_id, request_id)

                    if payment_result and payment_result.get("status") == "success":
                        # 4. Success Case: Commit transaction to our internal ledger
                        WalletService.adjust_balance(db, wallet.id, -total_amount)
                        ...
                        bill_tx = Transaction(
                            ...
                            description=f"Bill Payment: {biller_id} for {customer_id}",
                            additional_data=json.dumps({
                        ...
                        db.commit()
                        db.refresh(bill_tx)
                        return bill_tx  # Exit successfully
                    else:
                    ...
                    # Optionally, you could flag the provider as degraded here
                    # provider.status = "degraded_performance"
            else:
                logger.warning(f"Skipping unhealthy provider: {provider.name}")

        # 5. Failure Case: If the loop completes without a successful payment
...
class PayoutService:
    ...
    async def create_recipient(self, db: Session, user: User,
        ...
        """
        [REAL SYSTEM] Creates a Transfer Recipient on Paystack and saves the reference.
        """
        payload = {
            "type": "nuban",  # Or "mobile_money" for Ghana
        ...
        try:
            ...
            raise HTTPException(status_code=400,
                                detail=f"Could not verify or save bank account details. Reason: {str(e)}")
...
class NotificationService:
    ...
    def send_transaction_notification(db: Session, transaction_id: str):
        """Sends email notifications for successful transactions using the EmailService."""
        transaction = db.query(Transaction).get(transaction_id)
        if not transaction or transaction.status != TransactionStatus.COMPLETED:
            return

        # Use asyncio.create_task to run these concurrently in the background
        if transaction.sender:
            sender_html = f"""
            <h3>Transaction Successful</h3>
            <p>Hello {transaction.sender.full_name},</p>
            <p>Your transfer of <strong>{transaction.amount} {transaction.currency_code}</strong> to {transaction.receiver.email} was successful.</p>
            <p>Transaction ID: {transaction.id}</p>
            """
            asyncio.create_task(
                email_service.send_email(transaction.sender.email, "QuantumPay Transaction Notification", sender_html))

        if transaction.receiver:
            receiver_html = f"""
            <h3>You've Received a Payment!</h3>
            <p>Hello {transaction.receiver.full_name},</p>
            <p>You have received <strong>{transaction.amount} {transaction.currency_code}</strong> from {transaction.sender.email}.</p>
            <p>The funds are now available in your wallet.</p>
            """
            asyncio.create_task(
                email_service.send_email(transaction.receiver.email, "You've Received a Payment!", receiver_html))
...
class VirtualAccountService:
    ...
    Handles the creation and management of dynamic virtual accounts via a provider
    like Monnify or Providus Bank.
    """

    def __init__(self):
        # These would be in your .env file
        self.provider_api_key = "YOUR_MONNIFY_API_KEY"
        self.provider_secret_key = "YOUR_MONNIFY_SECRET_KEY"
        self.provider_base_url = "https://sandbox.monnify.com/api"  # Use live URL in production
        self.contract_code = "YOUR_MONNIFY_CONTRACT_CODE"

    async def _get_token(self) -> str:
        # Monnify uses Basic Auth with API Key:Secret Key for tokens
        # A real implementation would cache this token.
        auth_str = base64.b64encode(f"{self.provider_api_key}:{self.provider_secret_key}".encode()).decode()
        headers = {"Authorization": f"Basic {auth_str}"}
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(f"{self.provider_base_url}/v1/auth/login", headers=headers)
                response.raise_for_status()
                return response.json()["responseBody"]["accessToken"]
        except Exception as e:
            logger.error(f"Failed to get Monnify auth token: {e}")
            raise ConnectionError("Could not authenticate with virtual account provider.")

    async def provision_virtual_account(self, db: Session, wallet: Wallet) -> Wallet:
        """
        Calls the provider to generate and reserve a new virtual account
        and saves it to the wallet record.
        """
        if wallet.virtual_account_number:
            return wallet  # Account already exists
        ...
        payload = {
            "accountReference": wallet.id,  # Link the virtual account to our internal wallet ID
            "accountName": f"QuantumPay - {user.full_name}",
            "currencyCode": "NGN",
            "contractCode": self.contract_code,
            "customerEmail": user.email,
            "customerName": user.full_name,
            "getAllAvailableBanks": False,
            "preferredBanks": ["058"]  # Wema Bank is a common choice, code is 035 in some docs
        }

        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(f"{self.provider_base_url}/v2/bank-transfer/reserved-accounts",
                                             headers=headers, json=payload)
                response.raise_for_status()
                data = response.json()["responseBody"]

            # Update our wallet record with the details from the provider
            account = data["accounts"][0]
            wallet.virtual_account_number = account["accountNumber"]
            wallet.virtual_account_bank_name = account["bankName"]
            wallet.virtual_account_provider = "monnify"
            db.commit()
            db.refresh(wallet)

            logger.info(
                f"Successfully provisioned virtual account {wallet.virtual_account_number} for wallet {wallet.id}")
            return wallet

        except Exception as e:
            logger.error(f"Failed to provision virtual account for wallet {wallet.id}: {e}")
            # We don't raise an HTTPException here because the user can still use other methods.
            # We just return the wallet without the account details.
            return wallet
...
class AnalyticsService:
    ...
    def get_income_expense_chart_data(db: Session, user: User, months: int = 6) -> IncomeExpenseChartData:
        ...
        SQL query to generate data points for the dashboard's income vs. expense chart.
        """
        ...
        # It groups all transactions by year and month.
        # For each group, it conditionally sums the amounts based on whether the user
        ...
        # The query only returns months with activity. We need to fill in the gaps.
        # We'll create a dictionary of all months in the range and populate it.
        report_data = {}
        for i in range(months):
            # Iterate backwards from the current month
        ...
        # Sort the keys to ensure chronological order and format the output
        sorted_months = sorted(report_data.keys(), reverse=True)
        for year, month in sorted_months[:months]:
        ...
        # The data is in reverse chronological order, so we reverse it back for the chart
        data_points.reverse()
...
class USSDService:
    ...
    async def process_request(self) -> str:
        ...
        if session.current_menu == "send_money_enter_phone":
            ...
            if not recipient:
                return self._format_response(f"User with phone number {recipient_phone} not found. Please try again.",
                                             is_end=True)
...
async def disable_2fa(
    ...
    if TwoFactorService.verify_code(current_user.totp_secret, request.totp_code):
        ...
        raise HTTPException(status_code=400, detail="Invalid 2FA code. Disabling failed.")
...
async def setup_business_profile(
    ...
    new_profile = BusinessProfile(**profile_data.dict(), user_id=current_user.id)
    db.add(new_profile)
    db.commit()
...
async def create_invoice(
    ...
    tax_amount = total_amount * (invoice_data.tax_rate_percent / 100)
    ...
    new_invoice = Invoice(
        ...
        amount=final_amount,
        currency=invoice_data.currency.upper(),  # V4.6 - Capture currency
...
async def submit_expense(
    ...
    # Find the business this user is an employee of. A user could be part of multiple businesses.
    # We'll assume the first one found for simplicity.
    employment = db.query(Employee).filter(Employee.user_id == current_user.id).first()
    if not employment:
        raise HTTPException(status_code=403, detail="You are not an employee of any business on QuantumPay.")
    ...
    new_expense = Expense(
        ...
        amount=expense_data.amount or ocr_results['amount'],
        currency=expense_data.currency or ocr_results['currency'],
        merchant_name=expense_data.merchant_name or ocr_results['merchant_name'],
    )
    db.add(new_expense)
    db.commit()
...
# Mock job listings
mock_jobs = [
    JobListing(id="eng001", title="Senior Backend Engineer (Python)", location="Remote", department="Engineering",
               commitment="Full-time", description="Build the future of QuantumPay's core infrastructure.",
               apply_url="#"),
    JobListing(id="eng002", title="Lead Frontend Engineer (React)", location="New York, NY", department="Engineering",
               commitment="Full-time", description="Lead the development of our user-facing applications.",
               apply_url="#"),
    JobListing(id="prod001", title="Product Manager, AI & Fraud", location="Remote", department="Product",
               commitment="Full-time", description="Define the roadmap for our next-generation AI security features.",
               apply_url="#"),
]

# Mock blog posts
mock_posts = [
    BlogPost(id="bp001", title="QuantumPay V4.2: The Enterprise Upgrade", author="Jane Doe",
             publication_date=datetime.utcnow(), read_time_minutes=5, tags=["product", "update"],
             image_url="https://picsum.photos/seed/post1/800/400",
             summary="Announcing the most powerful version of QuantumPay yet, with features for businesses and developers.",
             content="<p>Full blog content goes here...</p>"),
    BlogPost(id="bp002", title="Understanding Our AI-Powered Fraud Shield", author="John Smith",
             publication_date=datetime.utcnow() - timedelta(days=7), read_time_minutes=7, tags=["security", "ai"],
             image_url="https://picsum.photos/seed/post2/800/400",
             summary="A deep dive into the technology that keeps your transactions safe.",
             content="<p>Full blog content goes here...</p>"),
]


@content_router.get("/jobs", response_model=List[JobListing])
async def get_job_listings():
    # In a real system, this would query a 'jobs' table in the database.
    return mock_jobs


@content_router.get("/blog-posts", response_model=List[BlogPost])
async def get_blog_posts():
    # In a real system, this would query a 'blog_posts' table or a headless CMS.
    return mock_posts


@content_router.get("/blog-posts/{post_id}", response_model=BlogPost)
...
async def get_system_status(db: Session = Depends(get_db)):
    ...
    try:
        auth.get_user_by_email("test@example.com")  # A non-existent user to test connectivity
    ...
    # Check AI Engine
    try:
        ai_engine.predict_fraud_v2(Transaction(amount=10), User(), db)
        ai_status = SystemComponentStatus(name="AI Engine", status="operational",
                                          details="Model loaded and responsive.")
    except Exception as e:
        ai_status = SystemComponentStatus(name="AI Engine", status="degraded_performance", details=f"Model error: {e}")
...
async def get_or_create_virtual_account(
    ...
    """
    Fetches virtual account details for a wallet. If they don't exist,
    it triggers the provisioning process on-the-fly.
    """
    ...
    # Trigger provisioning if details are missing
    if not wallet.virtual_account_number:
        wallet = await virtual_account_service.provision_virtual_account(db, wallet)

    if not wallet.virtual_account_number:
        # This means provisioning failed
        raise HTTPException(status_code=503,
                            detail="Could not generate account details at this time. Please try again later.")
...
@payouts_router.post("/recipient", status_code=201)
async def create_payout_recipient(
    ...
    """
    Step 1: Create and save a new bank account recipient for payouts.
    """
    # We don't return the full object to the client, just a success message.
    # The client can re-fetch the list of saved accounts.
    await payout_service.create_recipient(db, current_user, request)
...
if __name__ == "__main__":
    ...
    # To run this app:
    # 1. Set environment variables for SUPERUSER_EMAIL and SUPERUSER_FIREBASE_UID.
    #    You can get the Firebase UID after signing up a user in your Firebase project.
    #    Example:
    #    export SUPERUSER_EMAIL="your_su_email@example.com"
    #    export SUPERUSER_FIREBASE_UID="your_firebase_uid_string"
